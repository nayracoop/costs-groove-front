<template>
  <aside :class="['bg-black/90 text-stone flex flex-col border-r border-black/80 flex-shrink-0 h-full transition-all', isMenuExpanded ? 'w-64' : 'w-16']">
    <!-- Menu Header -->
    <div
      class="h-16 flex items-center px-6 border-b border-black/70 flex-shrink-0 bg-black/80 cursor-pointer hover:bg-black transition-colors"
      @click="isMenuExpanded = !isMenuExpanded"
    >
      <i :class="['fa-solid w-5 text-white mr-3', isMenuExpanded ? 'fa-chevron-left' : 'fa-chevron-right']" />
      <span
        v-if="isMenuExpanded"
        class="font-bold text-white tracking-wide text-sm leading-tight uppercase"
      >
        Centros de costos
      </span>
    </div>
    ...
    <div class="flex-1 overflow-y-auto py-4 transition-all">
      ...
      <NyrMenuSection>
        <NyrMenuLink
          ...
          :active="isActive('/grupos-productos')"
          to="/grupos-productos"
        />
        <NyrMenuLink
          icon="fa-calculator"
        ...
        <NyrMenuLink
          icon="fa-chart-area"
          label="Variable macro"
          :active="isActive('/variable-macro')"
          to="/variable-macro"
        />
      </NyrMenuSection>
      ...
      <h3
        v-if="isMenuExpanded"
        class="px-6 text-[10px] font-bold uppercase tracking-wider mb-2 text-white"
      >
        Gestión de Esquemas
      </h3>
    ...
    <NuxtLink
      to="/usuario"
      :class="['border-t border-black/70 py-4 bg-black/80 block hover:bg-black transition-colors cursor-pointer', isMenuExpanded ? 'px-6' : 'px-0 flex justify-center']"
    >
      <NyrAvatar
        initials="SN"
        name="Señorita Nayra"
...
watch(() => { return route.path; }, (newPath) => {
  if (newPath === "/tablero") {
