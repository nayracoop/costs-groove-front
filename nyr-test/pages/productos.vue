...
function buildProductoRow() {
  const unidadOption = unidadOptions.find((option) => { return option.key === newProductoForm.value.unidad; });
  const categoriaOption = categoriaOptions.find((option) => { return option.key === newProductoForm.value.categoria; });
