...
function buildInsumoRow() {
  const tipoOption = tipoOptions.find((option) => { return option.key === newInsumoForm.value.tipo; });
  const unidadOption = unidadOptions.find((option) => { return option.key === newInsumoForm.value.unidad; });
  const rubroOption = rubroOptions.find((option) => { return option.key === newInsumoForm.value.rubro; });
