...
function onCellEdited(event) {
  ...
  if (Array.isArray(cell.options)) {
    const match = cell.options.find((option) => { return option.key === newValue; });
    cell.value = newValue;
...
function buildAccountRow() {
  const planOption = planOptions.find((option) => { return option.key === newAccountForm.value.plan; });
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
      ...
      >
        ...
        <template #cell-admin-badge="{ cell }">
          <span
            v-if="cell.text"
            class="badge badge-admin-viewing"
          >
            <NyrIcon
              icon="eye"
              size="sm"
  ...
  >
    <div class="p-6">
      ...
      <p class="text-gray-600 mb-6">
        ¿Deseas archivar la cuenta <strong>{{ accountToArchive }}</strong>? Los datos seguiran disponibles en el archivo.
      </p>
